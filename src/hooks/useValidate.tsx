    ...
    const changeHandler = (e:React.ChangeEvent<HTMLInputElement>) => {
        ...
        switch(type.toLowerCase()){
            case 'email':
                ...
                else {setErrorMessage("Podany email jest nieprawidłowy"); setIsValid(false)}
                return
            case 'name':
                if (e.target.value.trim().length >= 3) setIsValid(true)
                else {setErrorMessage("Podane imię jest nieprawidłowe"); setIsValid(false)}
                return
            case 'password':
                if (e.target.value.trim().length >= 6) return setIsValid(true)
                else {setErrorMessage("Hasło powinno zawierać przynajmniej 6 znaków"); setIsValid(false)}
                return
            default:
                return
        }
